...
namespace Wasabimole.ProceduralTree
{
    ...
    public class ProceduralTree : MonoBehaviour
    {
        ...
        [Range(0f, 1f)]
        public float LeafProbability = 0.75f; // Leaf probability
        [Range(0f, 1f)]
        public float LeafMaximumRadius = 0.2f; // Maximum branch radeius where leaf can appear

        private int leavesVerticles = 0;
        private System.Random leafRandom = new System.Random();
        // ---------------------------------------------------------------------------------------------------------------------------
    ...
    public void GenerateTree()
    {
        ...
        void Branch(Quaternion quaternion, Vector3 position, int lastRingVertexIndex, float radius, float texCoordV)
        {
            ...
            radius *= RadiusStep;
            if (radius < MinimumRadius || vertexList.Count + NumberOfSides - leavesVerticles >= MaxNumVertices) // End branch if reached minimum radius, or ran out of vertices
            {
            ...
            //Add leaf
            if ( Random.value > ( 1f - LeafProbability) && LeafMaximumRadius > radius){
                AddLeaf( quaternion, position);
            }
            // Do we branch?
            if (vertexList.Count + NumberOfSides - leavesVerticles >= MaxNumVertices || Random.value > BranchProbability) return;
        ...
        private void AddLeaf(Quaternion quaternion, Vector3 position){
            leavesVerticles += 4;
            float rotation = (float) Mathf.PI * (float) leafRandom.NextDouble();
            var offset = Vector3.zero;
